...
var (
	...
	// The path-delay expression requires a non-zero value; the offset itself
	// may validly be zero. Regex: at least one non-zero digit.
...
func onPTPConfigChange(_ *interface{}, profile *ptpv1.PtpProfile) error {
	ifaces := upstreamPortsFromPtpProfile(profile)
	if len(ifaces) == 0 {
		return nil
	}
	...
	if err := updatePHC(profileName(profile), profile, ifaces, timeout); err != nil {
		return logFailure(profile, err)
	}
	return nil
}

func upstreamPortsFromPtpProfile(profile *ptpv1.PtpProfile) []string {
	...
	for _, rawLine := range strings.Split(*profile.Ptp4lConf, "\n") {
		...
		if strings.HasPrefix(line, "[") && strings.HasSuffix(line, "]") {
			...
			switch "[" + section + "]" {
			case config.Ptp4lGlobalSectionName, config.Ptp4lNmeaSectionName, config.Ptp4lUnicastSectionName:
				section = ""
			}
			continue
		}
...
func logFailure(profile *ptpv1.PtpProfile, err error) error {
	glog.Errorf("PHC sync workaround failed for profile %s: %v", profileName(profile), err)
	return err
}
...
func updatePHC(name string, profile *ptpv1.PtpProfile, ifaces []string, timeout time.Duration) error {
	ctx := context.Background()
	glog.Infof("PHC sync workaround started: profile=%s interfaces=%v", name, ifaces)
	devs, err := phcDevices(ifaces)
	if err != nil {
		return err
	}
	glog.Infof("PHC sync workaround resolved: profile=%s interfaces=%v PHCs=%v", name, ifaces, devs)

	offset, err := measureOffset(ctx, profile, ifaces, timeout)
	if err != nil {
		return fmt.Errorf("measure PHC offset for interfaces %v: %w", ifaces, err)
	}
	for _, dev := range devs {
		...
		if err != nil {
			return fmt.Errorf("calculate corrected PHC time for %s: %w", dev, err)
		}
		if err := setPHCTime(ctx, dev, corrected); err != nil {
			return fmt.Errorf("set PHC %s: %w", dev, err)
		}
	}
	glog.Infof("PHC sync workaround completed: profile=%s interfaces=%v PHCs=%v", name, ifaces, devs)
	return nil
}
...
func measureOffset(ctx context.Context, profile *ptpv1.PtpProfile, ifaces []string, timeout time.Duration) (int64, error) {
	...
	configFile, err := os.CreateTemp("", "phc-sync-*.conf")
	if err != nil {
	...
	if len(samples) == 0 {
		if timeout > 0 && measureCtx.Err() != nil {
			return 0, fmt.Errorf("timed out after %s waiting for a valid ptp4l offset sample", timeout)
		}
	...
	for _, rawLine := range strings.Split(*profile.Ptp4lConf, "\n") {
		...
		if len(fields) > 0 {
			...
			case "free_running", "slaveOnly", "uds_address", "uds_ro_address":
				continue
			case "domainNumber":
				if section == "global" && len(fields) == 2 {
					domain = fields[1]
				}
				continue
			}
